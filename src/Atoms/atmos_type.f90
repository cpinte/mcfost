...
!   type (GridType), target :: atmos
  ...
  subroutine compute_angular_integration_weights(method,Nmu,Nphi)
	integer, intent(in), optional :: Nmu, Nphi
	character(len=*), intent(in), optional :: method
	integer :: N1, N2, iray,iphi,imu, alloc_status, Nquad
	character(len=20) :: angular_method
	...
	if (present(method)) then
		angular_method = method
	else
		angular_method = "gauss_legendre"
	endif
	
	select case (angular_method)
...
!        0.01130115, 0.01602704, 0.01130115, 0.01130115, 0.01602704/)
	...
	case default
		...
		call error("Anuglar integration method unknown!", angular_method)
