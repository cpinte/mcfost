module broad
   ...
   function line_damping(icell, line)
      ...
      !unit less
      line_damping = line_damping * radfreq_to_vel / line%atom%vbroad(icell)
   ...
   subroutine VanderWaals_line(icell, line, GvdW)
      ...
      ! ! Helium basic informations
      Helium_elem => Elements(2)%ptr_elem
      ...
      vrel35_He = (8.*KBOLTZMANN/(PI*AMU*line%atom%weight) * (1.+line%atom%weight/Helium_elem%weight))**0.3
      ...
      C625 = (2.5*((Q_ELECTRON)**2./fourPIeps0)*(ABARH/fourPIeps0) * 2.*PI*(Z*RBOHR)**2./HPLANCK * deltaR)**(0.4)
      ...
      select case (line%vdWaals)
         case ("UNSOLD")
            ...
            vrel35_H = (8.*KBOLTZMANN/(PI*AMU*line%atom%weight) * (1.+line%atom%weight/Hydrogen%weight))**(0.3)
            cross = 8.08 * (line%cvdWaals(1)*vrel35_H+line%cvdWaals(3)*Helium_elem%abund*vrel35_He)*C625
   ...
   subroutine Stark_line(icell,line, GStark)
      ...
      if ( line%cStark < 0.) then
         ...
         C = 8. * KBOLTZMANN / (PI*AMU*line%atom%weight)
         ...
         ERYDBERG = E_RYDBERG / (1.+M_ELECTRON / (line%atom%weight*AMU)) !corection
