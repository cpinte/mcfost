...
CONTAINS
  ...
  function line_damping(icell, line)
    ...
    line_damping = line_damping * cDop / line%atom%vbroad(icell)
  ...
  SUBROUTINE VanderWaals_line(icell, line, GvdW)
    ...
    CASE ("UNSOLD")
       ! Relative velocity of radiator and perturber with Maxwellian
       ! velocity distributions
       vrel35_H = (8.*KBOLTZMANN/(PI*AMU*line%atom%weight) * (1.+line%atom%weight/Hydrogen%weight))**(3d-1)
    ...
    CASE ("BARKLEM")
       write(*,*) "Warning-> vdWaals broadening following BARKLEM ",&
            "not tested yet!"
       ! UNSOLD for Helium
       cross = 8.08 * line%cvdWaals(3)*Helium_elem%abund*vrel35_He*C625
       GvdW = line%cvdWaals(1) * T(icell)**(real(1.-line%cvdWaals(2),kind=8)/2.) + cross*T(icell)**(3d-1)
    CASE DEFAULT
       write(*,*) "Method for van der Waals broadening unknown"
       write(*,*) "exiting..."
       stop
