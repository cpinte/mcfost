...
!     nHtot = 1.045714d16
     ...
     do j=j_start,nz
      do k=1, n_az
       ...
       if (r >= Rmin .and. r<= 10) then
        ...
        nHtotr(icell) = nH0! * dsqrt(r0/r)
        Vr(icell) = 1d3 * vinf * (r/r0)**beta
  ...
  integer :: izone, i, j, k, icell, n_zones=1, n, NDEP
  double precision, dimension(n_cells) :: Vr, Tr,vturbr, nHtotr, ner
  double precision, dimension(n_cells) :: Scale_star_disk
  type(disk_zone_type) :: dz ! to define the properties of the model
  integer, parameter :: MAX_NDEP = 82!
  double precision, parameter :: f = 1d0 !1d0 to no change density
  double precision, dimension(MAX_NDEP) :: Tmarcs, nHmarcs, Rmarcs, vtmarcs, nemarcs
  double precision :: r, vt, rcyl, z, v0, alpha, b = 1d0, rin, rmin, rmax, Rstarmarcs
