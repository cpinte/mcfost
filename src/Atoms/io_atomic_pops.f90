...
CONTAINS
  ...
  subroutine prepare_check_pointing()
    ...
    return
  end subroutine prepare_check_pointing
  ...
  subroutine read_pops_atom(atom)
    ...
    do l=1,atom%Nlevel
       ...
       do icell=1,n_cells
          if (icompute_atomRT(icell) > 0) then!if on the exact grid (computed) is not empty but the interpolations are, empty the grid point.
