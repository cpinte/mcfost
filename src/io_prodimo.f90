          ...
          do lambda=1,n_lambda
             do l= grain_RE_LTE_start, grain_RE_LTE_end
                opacite(ri,zj,1,lambda) = opacite(ri,zj,1,lambda) + C_ext(l,lambda) * &
                     dust_density(l,icell) * nbre_grains(l) * facteur
          ...
          do ri=1,n_rad
             ...
             do lambda=1,n_lambda
                do l= iPAH_start, iPAH_end
                   opacite(ri,zj,1,lambda) = opacite(ri,zj,1,lambda) + C_ext(l,lambda) * dust_density(l,icell) * nbre_grains(l)
                   opacite(ri,zj,2,lambda) = opacite(ri,zj,2,lambda) + C_abs(l,lambda) * dust_density(l,icell) * nbre_grains(l)
                enddo ! l
             enddo ! lambda
          enddo ! ri
       enddo !zj
       ...
       do zj=1,nz
          do ri=1,n_rad
             ...
             do l= iPAH_start, iPAH_end
                ...
                TPAH_eq(ri,zj,1) = TPAH_eq(ri,zj,1) + Ttmp**4 * dust_density(l,icell) * nbre_grains(l)
                norme = norme + dust_density(l,icell) * nbre_grains(l)
             enddo ! l
