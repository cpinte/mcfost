...
contains
  ...
  SUBROUTINE setup_pluto_to_mcfost(pluto_file)
    ...
    integer 							:: Nmod
    real(dp), allocatable, dimension(:) :: x,y,z,h!allocated in reading
    real(dp), allocatable, dimension(:) :: vx,vy,vz,rho


    if (lpluto_file) then
       write(*,*) "Performing pluto to mcfost setup..."
       !to date, it is only reading a ascii file generated by mcfost
       CALL readAtmos_ascii("model.s",Nmod,x,y,z,h,rho,vx,vy,vz)
  ...
  !*********************************************************
  !building
  subroutine pluto_to_Voronoi(Nmod, x,y,z,h,rho,vx,vy,vz,check_previous_tesselation)
    ...
    real(dp), dimension(Nmod), intent(in) :: vx, vy, vz, rho
    logical, intent(in) :: check_previous_tesselation
    real(dp), dimension(6) :: limits
    ...
    k = 1
    limits(1) = select_inplace(k,real(x))*1.2
    limits(3) = select_inplace(k,real(y))*1.2
    limits(5) = select_inplace(k,real(z))*1.2  !change a little bit the boundaries

    k = Nmod
    limits(2) = select_inplace(k,real(x))*1.2
    limits(4) = select_inplace(k,real(y))*1.2
    limits(6) = select_inplace(k,real(z))*1.2
    write(*,*) "# Model limits :"
    write(*,*) "x =", limits(1), limits(2)
    write(*,*) "y =", limits(3), limits(4)
    write(*,*) "z =", limits(5), limits(6)
