...
contains
  ...
  !*********************************************************
  !building
  SUBROUTINE setup_pluto_to_mcfost(pluto_file)

    use utils, only : read_comments
    use atmos_type, only : atmos, init_atomic_atmos
    use read_pluto

    character(len=*), intent(in) :: pluto_file
    integer, parameter :: iunit = 1
    real(dp), allocatable, dimension(:) :: x,y,z!allocated in reading

    if (lpluto_file) then !Fill atmos structure
       write(*,*) "Performing pluto to mcfost setup..."
       !to date, it is only reading a ascii file generated by mcfost
       CALL readAtmos_ascii("model.s",x,y,z) !stored in atmos, except x,y,z
       !convert to good units also, expected SI for atomic line transfer
       !except x,y,z in AU. Add spots
       !CALL  read_hdF5(pluto_file,x, y, z)
    else
     write(*,*) "Error, PLUTO HDF5 file expected !"
     stop
    end if
    ...
    ! Voronoi tesselation
    CALL pluto_to_Voronoi(atmos%Nspace, x,y,z, .false.)
    deallocate(atmos%Vxyz) !Voronoi cell used instead for that one

   atmos%T = 8000d0 !K
   atmos%nHtot = atmos%nHtot * 1d3 / masseH / atmos%avgWeight !expecting to read rho from
  ...
  !*********************************************************
  !building
  subroutine pluto_to_Voronoi(Nmod, x,y,z,check_previous_tesselation)
    ...
    real(dp), dimension(Nmod), intent(in) :: x,y,z
    real(dp), dimension(Nmod) :: h
    logical, intent(in) :: check_previous_tesselation
    real(dp), dimension(6) :: limits
    real :: f, density_factor
    integer :: icell, l, k, i, id_n, voroindex
    logical :: n_force_empty
...
!        if (iSPH > 0) then
!           masse_gaz(icell)    = massgas(voroindex) * Msun_to_g ! en g
!           densite_gaz(icell)  = masse_gaz(icell) /  (masse_mol_gaz * volume(icell) * AU3_to_m3)
!        else ! star
!           masse_gaz(icell)    = 0.
!           densite_gaz(icell)  = 0.
...
!     enddo
    ...
    if (lemission_atom) then
       do icell=1,n_cells
          voroindex = Voronoi(icell)%id
          if (voroindex > 0) then
             Voronoi(icell)%vxyz(1) = atmos%vxyz(voroindex,1)
             Voronoi(icell)%vxyz(2) = atmos%vxyz(voroindex,2)
             Voronoi(icell)%vxyz(3) = atmos%vxyz(voroindex,3)
          endif
       enddo
    endif
