...
!      checkpointing
   ...
   ! ------------------------------------------------------------------------------------ !
      integer :: iray, istep, i0, i1, n_rayons_max
      integer, parameter :: lorder_min = 0 !healpix
      integer, parameter :: imax_mc = 6 !th number of maximum rays in MC is n_Rayons_mc * 2**imax_mc
      ...
      logical :: lconverged, lprevious_converged
      ...
      if ((istep_start==1).and.(istep_end==1)) then
         lhealpix = .true.
      ...
      step_loop : do istep=i0, i1
         ...
         lconverged = .false.
         lprevious_converged = (lforce_lte).or.(istep < i1) !if lte or if not last steps.
         ...
         do while (.not.lconverged)
            ...
            ! 
            !                    goes with maxIter
            write(*,'(" *** Iteration #"(1I4)"; threshold: "(1ES11.2E3)"; Nrays: "(1I6))') &
            ...
            if (lng_acceleration) then
          	!be sure we are converging before extrapolating
               ...
               lconverging = (max(diff_old,dne) < 1.0)
      ...
      ! else
      !    write(100) 1.0/real(n_rayons_max,kind=dp)
      ! endif
      ! write(100) iloc
      ...
      return
   end subroutine nlte_loop_mali_2
