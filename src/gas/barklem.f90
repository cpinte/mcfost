...
module barklem
   ...
   subroutine init_Barklem_cross_data(btype, bs, res)
      ...
      integer :: j, i, n, Nread
      character(len=512) :: inputline, FomatLine !len=120
      ...
      do i=1,bs%N1+bs%N1
         if (i.le.bs%N1) then
            call read_line(2,FomatLine,inputline,Nread,commentchar=COMMENT_CHAR)
   ...
   subroutine get_Barklem_cross_data(atom, kr, res)
      ...
      mu = amu_kg/(1.0/atomic_weights(1) + 1./atom%weight)
      vmean = sqrt(8.0*kb/pi/mu) !missing a sqrt(T)
      ...
      if (atom%lines(kr)%cvdWaals(1) > 1.0) then
         ...
         E_Rydberg2 = E_RYDBERG / (1. + mel/(atom%weight*amu_kg) )
         ...
         atom%lines(kr)%cvdWaals(1) = &
            bilinear(bs%N1,bs%neff1,bs%N2,bs%neff2,bs%cross,neff1,neff2)

         !get exponent
         ! atom%lines(kr)%cvdWaals(2) = &
         !    interp2D(bs%neff1,bs%neff2,bs%alpha,neff1,neff2)
         atom%lines(kr)%cvdWaals(2) = &
            bilinear(bs%N1,bs%neff1,bs%N2,bs%neff2,bs%alpha,neff1,neff2)
