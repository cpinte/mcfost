  ...
  do i=1, n_pop
     ...
     if (abs(dp%amin - dp%amax) < 1.0e-5 * dp%amax) then
        ...
        if (abs(dp%aexp - 4.) > 1.0e-5) then
           dp%avg_grain_mass = quatre_tiers_pi * mum_to_cm**3 * dp%rho1g *(1-dp%aexp)/(4-dp%aexp)*(dp%amax**(4-dp%aexp)-dp%amin**(4-dp%aexp)) / &
                (dp%amax**(1-dp%aexp)-dp%amin**(1-dp%aexp))
        else
           dp%avg_grain_mass = quatre_tiers_pi * mum_to_cm**3 * dp%rho1g *(1-dp%aexp)*(log(dp%amax)-log(dp%amin)) / &
                (dp%amax**(1-dp%aexp)-dp%amin**(1-dp%aexp))
        endif
     endif
