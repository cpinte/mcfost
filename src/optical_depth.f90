...
subroutine tau_deg2(id,lambda,E,ri,zj,xio,yio,zio,u,v,w,lin,tau_out,flag_sortie)
! Integration par calcul de la position de l'interface entre cellules
! par eq deg2 en r et deg 1 en z
! Ne met a jour xio, ... que si le photon ne sort pas de la nebuleuse (flag_sortie=1)
! C. Pinte
! 05/02/05
  ...
  do ! Boucle infinie
     ...
     ! Comparaison integrale avec tau
     if(ltot > lin) then ! On a fini d'integrer
        tau = tau*lin/ltot ! on rescale tau pour que ltot=lin
        tau_out= tau_out + tau
        flag_sortie = .false.
        xio=x0+l*u
        yio=y0+l*v
...
end subroutine tau_deg2

!********************************************************************

subroutine length_dic(id,lambda,E,ri,zj,xio,yio,zio,u,v,w,extrin,ltot,flag_sortie)
! Calcule la longueur de vol associee a la profondeur optique
! tiree aleatoirement.
!
! flag = 0 -> le photon est toujours dans l'enveloppe
! flag = 1 -> on a integre toute la densite de colonne
! flag = 2 -> le photon atteint le bord de l'enveloppe, et va
!     peut-etre sortir (statut momentane seulement, le temps de
!     calculer l'integrant jusqu'a size)
! flag = 4 -> le photon sort sans integrer toute la densite de colonne
...
! reprise :  1/03/06 : on ne recacule plus l'indice de cellule on fait des indentations : BUG + semble + long que deg2
  ...
  real(kind=db) :: x, y, z, delta, extr, x0, y0, z0, x1, y1, z1, x2, y2, z2
...
!  write(*,*) "entree", u, v, w, sqrt(u*u+v*v+w*w)
  ...
  do ! Boucle infinie
   ...
   !  write(*,*) ri0, zj0, sqrt(x*x+y*y) , z
     ...
     nbr_cell = nbr_cell + 1
     ! Le photon continue dans cette cellule
     if (ri0==0) then
        lcellule_non_vide=.false.
        opacite=0.0
        delta= delta0(0)
        ...
        opacite=kappa(lambda,ri0,zj0,1)
        ! Pas initial d'integration
        delta = delta0(ri0)
        r2_inf=r_lim_2(ri0-1)
        r2_sup=r_lim_2(ri0)
        ! le photon s'éloigne ou pas du plan median
        dotprod=z0*w
        if (z>0.0) then
           z_inf=z_lim(ri0,zj0)
           z_sup=z_lim(ri0,zj0+1)
        else
           z_inf=-z_lim(ri0,zj0+1)
           z_sup=-z_lim(ri0,zj0)
        endif
     endif
  ...
  !      write(*,*) sqrt(r2)
   ...
   !  write(*,*) "res tmp", sqrt(r2), z


     ! Boucle dichotomie : on cherche le bord de la cellule
     x1=x_old ; y1=y_old ; z1=z_old
     x2=x ; y2=y; z2=z
   ...
   !     write(*,*) "b"
    ...
    ! changement z
     else if (z > z_sup) then
  !      write(*,*) "c"
        if (dotprod > 0.0) then !on s'eloigne du plan
           zj1=zj0+1
           ri1=ri0
        else ! on s'approche du plan
           if (zj0==1) then ! on traverse le plan
              zj1=zj0
           else
              zj1=zj0-1
           endif
 ...
 !       write(*,*) "d"
  ...
  !  read(*,*)
     ...
     if(tau > extr) then ! On a fini d'integrer
        ...
        ltot=ltot+l
        if (letape_th.and.lcellule_non_vide) then
           if (lRE_LTE) xKJ_abs(ri0,zj0,1,id) = xKJ_abs(ri0,zj0,1,id) + kappa_abs_eg(lambda,ri0,zj0,1) * l * E
           if (lxJ_abs) xJ_abs(lambda,ri0,zj0,id) = xJ_abs(lambda,ri0,zj0,id) + l * E
        endif !l_abs
        flag_sortie = .false.
        xio=x0+l*u
        yio=y0+l*v
  ...
  enddo ! boucle infinie
! on n'a atteint ni la borne de l'integrale, ni le bord de
! l'enveloppe
  write (*,*) 'BUG'
end subroutine length_dic
...
subroutine length_deg2_2nd(id,lambda,E_tot,E_trans,ri_io,zj_io,xio,yio,zio,u,v,w,tau,dvol,flag_sortie)
! Recherche position interaction apres length_deg2_tot
! en minimisant les calculs (evite d'utiliser length_deg2)
...
! 17/08/05
  ...
  integer, intent(in) :: id, lambda
  logical, intent(out) :: flag_sortie
  integer, intent(inout) :: ri_io, zj_io
  real(kind=db), intent(in) :: E_tot, E_trans
  real(kind=db), intent(in) :: u, v, w
  real, intent(in) :: tau
  real(kind=db), intent(inout) :: xio, yio, zio
  ...
  kmin=0
  kmax=n_cell_traversees(id)
  k=(kmin+kmax)/2

  if (tau > tab_tau(id,kmax)) then ! Pas interaction
     flag_sortie = .true.
  else ! interaction
     flag_sortie = .false.

     do while ((kmax-kmin) > 1)
        if (tab_tau(id,k) < tau) then
           kmin = k
        else
           kmax = k
        endif
     ...
     enddo   ! while
     k=kmax
     ! on est entre kmax-1 et kmax

     ! Position interaction
     l = tab_length(id,kmax) * (tau-tab_tau(id,kmax-1))/(tab_tau(id,kmax)-tab_tau(id,kmax-1)) !tab_tau(id:0)=0.0
     xio=tab_x0(id,kmax) + u*l
     yio=tab_y0(id,kmax) + v*l
     zio=tab_z0(id,kmax) + w*l

     ri_io=tab_cell_r(id,kmax)
     zj_io=tab_cell_z(id,kmax)


     dvol=0.0

     ! Cellules avant interaction
     do k=1,kmax-1
        dvol=dvol+tab_length(id,k)
     enddo
     ! Cellule ou a lieu l'interaction : kmax
     dvol=dvol+l
  endif

  return

end subroutine length_deg2_2nd

!***********************************************************

subroutine tau_deg2_2nd(id,lambda,E,ri_io,zj_io,xio,yio,zio,u,v,w,lin,tau_out,flag_sortie,n_cell_diff)
! Recherche tau interaction apres length_deg2_tot
! en minimisant les calculs (evite d'utiliser tau_deg2)
! grâce aux infos sauvées dans length_deg2_tot
! C. Pinte
! 17/06/06
  ...
  integer, intent(in) :: id, lambda
  logical, intent(out) :: flag_sortie
  integer, intent(out) :: n_cell_diff
  integer, intent(inout) :: ri_io, zj_io
  real, intent(in) :: lin, E
  real(kind=db), intent(in) :: u, v, w
  real(kind=db), intent(inout) :: xio, yio, zio
  real, intent(out) :: tau_out
  ...
  if (lin > tab_length_tot(id,kmax)) then ! Pas interaction
     flag_sortie = .true.
  else ! interaction
     flag_sortie = .false.

     do while ((kmax-kmin) > 1)
        if (tab_length_tot(id,k) < lin) then
           kmin = k
        else
           kmax = k
        endif
...
end subroutine tau_deg2_2nd

!***********************************************************

subroutine surface_tau()
! calcul surface tau=1
  ...
  integer, parameter :: n_point = 1000
  real, parameter :: tau_max = 1.0
  real, parameter :: wavelength = 0.3
...
100  format("#    r              h1              h	")
  open(unit=1,file="data_"//trim(band)//"/surface.txt",status="new")
  write(1,100)
  ...
  do i=1, n_point
     ...
     if (flag_sortie) then
        ...
        v1=0.
        zj=nz+1
        call length_deg2(id,lambda,Stokes,ri,zj,phik,x1,y1,z1,u1,v1,w1,flag_star,flag_direct_star,tau_max,dvol1,flag_sortie)
