...
extern "C" {
  void voro_C_with_wall_db(int n, int max_neighbours, double limits[6], double x[], double y[], double z[], double h[],  double threshold, int n_vectors, double cutting_vectors[][3], double cutting_distance_o_h, int icell_start, int icell_end, int cpu_id, int n_cpu, int n_points_per_cpu,
              int &n_in, double volume[], int first_neighbours[], int last_neighbours[], int n_neighbours[], int neighbours_list[], double xs, double ys, double zs, double rs, bool was_cell_cut[],int &ierr) {
    ...
    //Add a wall at the location of the star. Only one accepted yet.
	//wall_spherical sphere_wall(xs, ys, zs, rs, w_id_=-99);// !id of the wall for neighbour tracking
	//con.add_wall(sphere_wall);
