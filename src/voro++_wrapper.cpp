              ...
              int &n_in, double volume[], int first_neighbours[], int last_neighbours[], int n_neighbours[], int neighbours_list[], bool was_cell_cut[], int n_stars, int stellar_id[], double stellar_radius[], bool stellar_neighb[], int &ierr)
  {
    ...
    */
    ...
    do
    {
      ...
      { //it is a star !
        ...
        if (!find_at_least_one_star)
          find_at_least_one_star = true;
        for (i = 0; i < c.number_of_faces(); i++)
        {
          if (vi[i] >= 0)
          {                          // Flag neighbour as neighbour of stars, giving the id of the stellar cell.
    ...
    do
    {
      ...
      { // We only compute a fraction of the cells in a given thread
        if (con.compute_cell(c, vlo))
        { // return false if the cell was removed
          pid_loc++;
          ...
          first_neighbours[pid] = first_neighbour;
          last_neighbour = last_neighbour + n_neighbours_cell;
          last_neighbours[pid] = last_neighbour;
    ...
    //free temporary storage and leave.
    free(has_a_star);
    return;
  }
}
