...
module HDF5_utils
  ...
  ! Compatibility API from former utils_hdf5 module
  public :: write_to_hdf5, read_from_hdf5
  public :: open_hdf5file, create_hdf5file, close_hdf5file
  public :: open_hdf5group, create_hdf5group, close_hdf5group
  ...
  !> Compatibility interface: write dataset with (x, name, id, error) calling convention
  interface write_to_hdf5
   module procedure write_real_kind4,              & ! real(4)
                    ...
                    write_hdf5_string                ! strings
  end interface write_to_hdf5

  !> Compatibility interface: read dataset with (x, name, id, got, error) calling convention
...
contains
  ...
  !!---------------------------------------------------------------------------------------
  !!--- Compatibility API from former utils_hdf5 module (Phantom HDF5 interface) ---------
  !!---------------------------------------------------------------------------------------
  ...
  subroutine create_hdf5file(filename,file_id,error)
   ...
   integer :: errors(2)
   call h5open_f(errors(1))
   call h5fcreate_f(filename,H5F_ACC_TRUNC_F,file_id,errors(2))
  ...
  subroutine open_hdf5file(filename,file_id,error)
   ...
   integer :: errors(2)
   call h5open_f(errors(1))
   call h5fopen_f(filename,H5F_ACC_RDWR_F,file_id,errors(2))
   error = maxval(abs(errors))
  ...
  subroutine close_hdf5file(file_id,error)
   ...
   integer,        intent(out) :: error
   integer :: errors(2)
   call h5fclose_f(file_id,errors(2))
   call h5close_f(errors(1))
   error = maxval(abs(errors))
  end subroutine close_hdf5file

  subroutine create_hdf5group(loc_id,group_name,group_id,hdferror)
   integer(HID_T), intent(in)  :: loc_id
   character(len=*), intent(in) :: group_name
   integer(HID_T), intent(out) :: group_id
   integer, intent(out) :: hdferror
   call h5gcreate_f(loc_id, group_name, group_id, hdferror)
  end subroutine create_hdf5group

  subroutine open_hdf5group(loc_id,group_name,group_id,hdferror)
   integer(HID_T), intent(in)  :: loc_id
   character(len=*), intent(in) :: group_name
   integer(HID_T), intent(out) :: group_id
   integer, intent(out) :: hdferror
  ...
  subroutine close_hdf5group(group_id,hdferror)
  ...
  subroutine write_hdf5_string(str,name,id,error)
  ...
  !!--- read_from_hdf5 implementations ---
  ...
  subroutine read_hdf5_string(str,name,id,got,error)
