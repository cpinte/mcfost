...
!    if (trim(VariableNames) /= "rho") call error("mcfost cannot read VariableNames in athena++ dump")
  ...
  subroutine read_athena_model()
    ...
    integer :: ierr, alloc_status, n_variables, n_stars_old
    integer(HID_T) :: file_id, group_id
    ...
    real(dp) :: Ggrav_athena, umass, usolarmass, ulength, utime, udens, uvelocity, ulength_au, mass, factor
    ...
    star(2)%x = -x * ulength_au
    star(2)%y = -y * ulength_au
    star(2)%z =  z * ulength_au

    ! -vx and -y as phi is defined differently in fargo3d
    star(2)%vx = -vx * uvelocity
    star(2)%vy = -vy * uvelocity
    star(2)%vz =  vz * uvelocity

    star(2)%M = Mp * usolarmass
