...
subroutine alloc_dynamique()
  ...
  allocate(tab_lambda(n_lambda), tab_delta_lambda(n_lambda), stat=alloc_status)
  if (alloc_status > 0) then
     write(*,*) 'Allocation error tab_lambda'
     stop
  endif
  tab_lambda=0.0; tab_delta_lambda=0.0
...
subroutine realloc_step2()
  ...
  deallocate(tab_lambda,tab_delta_lambda,tab_amu1,tab_amu2)
  allocate(tab_lambda(n_lambda2),tab_delta_lambda(n_lambda2),tab_amu1(n_lambda2,n_pop), &
       tab_amu2(n_lambda2,n_pop), stat=alloc_status)
  if (alloc_status > 0) then
     write(*,*) 'Allocation error tab_lambda in init_lambda2'
     stop
  endif
  tab_lambda=0.0
  tab_delta_lambda=0.0
  tab_amu1=0.0
  tab_amu2=0.0
...
subroutine alloc_emission_mol(imol)
  ...
  if (RT_line_method == 1) then
     allocate(spectre(-n_speed_rt:n_speed_rt,nTrans_raytracing,RT_n_ibin,1,1), &
             continu(nTrans_raytracing,RT_n_ibin,1,1), stat=alloc_status)
  else
     allocate(spectre(-n_speed_rt:n_speed_rt,nTrans_raytracing,RT_n_ibin,igridx,igridy), &
          continu(nTrans_raytracing,RT_n_ibin,igridx,igridy), stat=alloc_status)
  endif
  if (alloc_status > 0) then
...
subroutine dealloc_emission_mol()
  ...
  deallocate(kappa_mol_o_freq, emissivite_mol_o_freq, tab_nLevel, tab_nLevel_old, &
       tab_v, tab_deltaV, spectre, continu, tab_Cmb_mol, Jmol)
