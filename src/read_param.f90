  ...
  subroutine read_para()
    ...
    if (lfits) then
       ...
       do j=1,n_zones 
          ...
          if (j==1) then
             size_neb=size_neb_tmp
          else
             if (abs(size_neb-size_neb_tmp) > 1.e-6*size_neb) then
    ...
    if (rout > size_neb) then
       write(*,*)  "Erreur : Rout > size_neb"
       stop
    endif
    ...
    do i=1,n_cartes-1
       ...
       size_pix2(i)=maxigrid2(i)/(2.0*size_neb)
    end do
    ...
    if (lfits) then
       ...
       do j=1, n_zones
          ...
          read(1,*) dust_pop_tmp(n_pop)%lcoating, dust_pop_tmp(n_pop)%coating_frac, dust_pop_tmp(n_pop)%indices_coating, dust_pop_tmp(n_pop)%lmantle
        ...
        if (dust_pop_tmp(n_pop)%lmantle == .false.) then
