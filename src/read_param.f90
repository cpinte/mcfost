...
contains
  ...
  subroutine read_para(para)
    ...
    if (abs(para_version - 4.1) > 1.e-4) then
       ...
       if (abs(para_version-4.0) < 1.e-4) then
          ...
          close(unit=1)
          call read_para40(para)
          return
       else if (abs(para_version-3.0) < 1.e-4) then
    ...
    ! -----------------
    read(1,*) line_buffer
    !read(1,*) scattering_method0 ;
    scattering_method0 = 0
    scattering_method = scattering_method0
    read(1,*) aniso_method
  ...
  subroutine read_para3(para)
   ...
   ! Gas RT settings
   ! ---------------------
   !first read the abundance file for all species
