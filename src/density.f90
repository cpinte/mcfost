...
! Remplissage a zero pour z > zmax que l'en envoie sur l'indice j=0
  do i=1,n_rad
     do  k=1,n_grains_tot
        densite_pouss(cell_map(i,nz+1,1),k) = densite_pouss(cell_map(i,nz,1),k)
     enddo !k
  enddo !i
...
subroutine densite_file()
  ...
  search_not_empty : do l=1,n_grains_tot
     do k=1,n_az
        do j=1,nz
           do i=1,n_rad
              if (densite_pouss(cell_map(i,j,k),l) > 0.0_db) then
                 ri_not_empty = i
                 zj_not_empty = j
                 phik_not_empty = k
                 exit search_not_empty
...
subroutine densite_Seb_Charnoz()
  ...
  search_not_empty : do l=1,n_grains_tot
     do j=1,nz
        do i=1,n_rad
           if (densite_pouss(cell_map(i,j,1),l) > 0.0_db) then
              ri_not_empty = i
              zj_not_empty = j
              phik_not_empty = 1
              exit search_not_empty
...
subroutine densite_Seb_Charnoz2()
  ...
  do k=1,n_grains_tot
     do i=1, n_rad
        do j=1,nz
           densite_pouss(cell_map(i,j,1),k) = dens(i,j)
        enddo
     enddo
  enddo
